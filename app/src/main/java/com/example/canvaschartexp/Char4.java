...
public class Char4 extends View {
    public Char4(Context context) {
        super(context);
    }

    public Char4(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public Char4(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        cPaint.setShader(null);
        ...
        //底部扇形
        cPaint.setStrokeWidth(width / (7 + 7 / 10f));

//        cPaint.setShader(null);
        float r3 = width / 10f;
        RectF rectF3 = new RectF(r3, r3, width - r3, height - r3);
        canvas.drawArc(rectF3, 40, 100, false, cPaint);

        //圈内刻度扇形
        cPaint.setStrokeWidth(width / 20f);
        PathEffect effects2 = new DashPathEffect(new float[]{width / 14f, height / 28f, width / 14f, height / 28f}, 0);
        cPaint.setPathEffect(effects2);
        float r2 = width / (6 + 36 / 100f);
        RectF rectF2 = new RectF(r2, r2, width - r2, height - r2);
        canvas.drawArc(rectF2, 147, 249, false, cPaint);
        cPaint.setShader(mShader);
        canvas.drawArc(rectF2, 147, sweepangle, false, cPaint);

        //短刻度扇形
        float r4 = width / (17 + 1 / 2f);
        RectF rectF4 = new RectF(r4, r4, width - r4, height - r4);
        PathEffect effects3 = new DashPathEffect(new float[]{width / 350f, height / (116 + 6 / 10f), width / 350f, height / (116 + 6 / 10f)}, 0);
        cPaint.setPathEffect(effects3);
        cPaint.setShader(null);
        canvas.drawArc(rectF4, 144, 252, false, cPaint);

        /*可以变化角度，改变值，渐变刻度渐变内圆*/
        cPaint.setShader(mShader);
        canvas.drawArc(rectF4, 144, sweepangle+3, false, cPaint);
    ...
    private void drawLine(Canvas canvas) {
        ...
        canvas.drawLine(getMeasuredWidth() / 2, 0, getMeasuredWidth() / 2, getMeasuredHeight() * 1 / 10, lPaint);
        canvas.save();
        float degree = 144 / 8;
        for (int i = 0; i < 7; i++) {
    ...
    public void setTextNum(int textNum) {
        ...
        if (textNum<150){
            setSweepangle(textNum*(1+68/100f));
        }else {
            setSweepangle(249);
        }
    }
